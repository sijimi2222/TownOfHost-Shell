...

{
    ...
    // target1/target2で指定した相手が「追放」によって退場した場合、

    // OnReportDeadBody(死体通報)側のリセットしか無く、追放時のリセットが

    // 存在しなかったため、target1/target2やIsUseingが残ったまま

    // 次の会議に持ち越されてしまっていた。

    // この状態で次の会議に入ると、CheckVoteAsVoterのIsUseing分岐

    // (「ターゲット生存中は他のキルは不可」に相当する投票側の制限)が

    // 意図せず働き続け、実質的に投票が正常に機能しなくなり、

    // 全員同数表示のまま即終了する不具合につながっていた。

    // 追放によって対象が退場した場合も、次の会議に入る前に確実に
...

    {
        ...
        if (exiled.PlayerId == target1 || exiled.PlayerId == target2)
