...

    {
...

        {
...

            {
                ...
                //守護者(Guardian)のガードチェック

                if (GuardreasonNumber == -1 && Crewmate.Guardian.FindGuardianFor(attemptTarget.PlayerId) != null)
...

            {
...

                {
                    ...
                    case 5://Guardian(守護者)
                        ...
                        var guardian = Crewmate.Guardian.FindGuardianFor(attemptTarget.PlayerId);
...

                        {
                            ...
                            guardian.Sacrifice(attemptTarget);

                            UtilsGameLog.AddGameLog($"Guardian", UtilsName.GetPlayerColor(attemptTarget) + ":  " + string.Format(Translator.GetString("GuardMaster.Guard"), UtilsName.GetPlayerColor(attemptKiller, true)));
...
{//Default
    ...
    // 氷鬼(IceOni)は要望により役職として削除済み(実装ファイルIceOni.csを削除)。

    // enumの並び(数値)がズレるとセーブデータ等に影響するため、値自体はここに残している。
