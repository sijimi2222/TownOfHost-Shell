...

{
...

            {
...

                {
                    ...
                    if (SaruWarningOption.ShouldWarn(role) && !(role == CustomRoles.Amnesiac && Amnesiac.IsWolf))
                        ...
                        && Roles.AddOns.Common.Guesser.WarnIfCrewmate.GetBool()
                    ...
                    // ダークシェリフ本人はイントロでも通常のシェリフとして認識する。

                    else if (role == CustomRoles.DarkSheriff)

                    {

                        __instance.RoleText.text = UtilsRoleText.GetRoleName(CustomRoles.Sheriff);

                        __instance.RoleText.color = UtilsRoleText.GetRoleColor(CustomRoles.Sheriff);
...

    {
...

        {
            ...
            foreach (var o in OptionItem.AllOptions.Where(o => o is not ObjectOptionitem))
                ...
                if (!o.IsHiddenOn(Options.CurrentGameMode) && (o.Parent == null ? !o.GetString().Equals("0%") : o.IsParentValueEnabledForDisplay()) && o.IsEnabled.Invoke())
...

    {
...

        {
            ...
            if (PlayerControl.LocalPlayer.Is(CustomRoleTypes.Neutral) && !PlayerControl.LocalPlayer.Is(CustomRoles.BakeCat) && !PlayerControl.LocalPlayer.Is(CustomRoles.Amnesia) && !PlayerControl.LocalPlayer.Is(CustomRoles.DarkSheriff))
...

        {
            ...
            // ダークシェリフ本人は開始時からシェリフとして自覚・進行する。

            // イントロのチーム名、背景、説明をすべてクルー（シェリフ）用に統一する。

            if (role is CustomRoles.DarkSheriff) role = CustomRoles.Sheriff;
