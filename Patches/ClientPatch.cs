...
namespace TownOfHost
{
    ...
    class MakePublicPatch
    {
        public static bool Prefix(GameStartManager __instance)
        {
            ...
            if (!Main.IsPublicRoomAllowed())
            {
                ...
                else if (ModUpdater.isBroken) message = GetString("ModBrokenMessage");
                Logger.Info(message, "MakePublicPatch");
                Logger.seeingame(message);
                return false;
            }
    ...
    class CreateGameOptionsShowPatch
    {
        ...
        public static void Postfix(CreateGameOptions __instance)
        {
            ...
            // 部屋の自動立て直し中は、ここまで来たら「ゲーム作成」ボタンも自動で押す。
            if (AutoRehost.IsPending)
            {
                AutoRehost.ClickCreateGameButton();
            }
        }
    }
