...

{
...

        {
...

            {
...

                {
                    ...
                    // DuplicateConnectionDetected(多重接続によるキック)は、

                    // Character(PlayerControl)がまだ生成される前に切断されるのが正常な挙動のため、

                    // 「切断処理の失敗」として扱わない(ロビーでのエラー表示を出さない)。

                    if (reason == DisconnectReasons.DuplicateConnectionDetected)

                    {

                        Logger.Info($"多重接続として切断: {data?.PlayerName ?? "('ω')"}", nameof(OnPlayerLeftPatch));
...

    {
...

        {
                                    ...
                                    // 旧版の大きな歓迎バナーはTemplateManagerのwelcomeと内容が重複するため送信しない。

            // 入室時の案内はTemplateManager側の整理済み3メッセージへ統一する。

            // ただし、実施中アンケートは歓迎文とは別の機能のため維持する。

            if (SurveySystem.HasActiveSurvey)

            {

                Utils.SendMessage($"<size=60%><color=#00c1ff>ただいま「{SurveySystem.Current.Question}」のアンケートを行っております！\n「/cmd q」で一覧、「/cmd aq (番号)」で投票できますのでぜひお願いします！</color></size>", Sendto);
