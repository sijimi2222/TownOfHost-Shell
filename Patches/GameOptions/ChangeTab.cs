...

    {
...

        {
...

            {
                ...
                // Unityの破棄済みオブジェクトは "?." では正しく検知できない(生のnullとは違う)ため、

                // 明示的に "!= null" で比較する。部屋の自動立て直し後、古い参照が残ったまま

                // ここにアクセスしてクラッシュする不具合があった。

                if (ModSettingsTab != null) ModSettingsTab.gameObject.SetActive(false);

                if (ModSettingsButton != null) ModSettingsButton.SelectButton(false);
...

            {
                ...
                if (ModSettingsButton != null) ModSettingsButton.SelectButton(true);
