...

{
...

    {
...

        {
            ...
            // 対象選択中の本人クリックだけを候補選択として扱う。

            // 本人が自分へ投票して対象を確定した後は、天秤会議と同じ制限付き通常投票へ移行する。

            if (Reversal.IsInReversalMeeting(voter.PlayerId)

                && !Reversal.IsSelectionLocked(voter.PlayerId)
...

            {
                ...
                var guessedTarget = PlayerCatch.GetPlayerById(suspectPlayerId);

                Reversal.OnSelfVoteGuess(voter, guessedTarget);
    ...
    // V11公式ジャッジのOverruleはゲーム本体のJudgeOverruleVote／QueueOverruleVotesへ委ねる。
    // MeetingVoteManagerとの既存互換のためnonce保管場所だけを残し、未確認のHarmonyフックは登録しない。
...

            {
...

                {
...

                    {
                        ...
                        var data = GameData.Instance.AllPlayers.ToArray().Where(a => a.PlayerId == x.PlayerId).FirstOrDefault();
