...

{
...

    {
        ...
        private static int roleTextUpdateTick;

        private static readonly Dictionary<byte, int> lastRoleTextUpdateTick = new();
...

        {
...

            {
                ...
                // 全プレイヤーごとのFixedUpdateから呼ぶと同じ全員走査を毎フレーム繰り返すため、

                // ホスト自身の15tickごとの1回だけに集約する。

                if (AmongUsClient.Instance.AmHost && GameStates.IsInTask && !GameStates.IsLobby && timer is 0)

                    LastCrewmate.CheckAndGrantGuard();
            ...
            if (GameStates.InGame && !GameStates.Intro && timer is 0)

            {

                // 全員の外見復元は毎フレーム不要なため、役職表示の更新周期に合わせて間引く。
...

            {
...

                {
                    ...
                    bool shouldRefreshRoleText = !lastRoleTextUpdateTick.TryGetValue(__instance.PlayerId, out var lastTick)
                        ...
                        || roleTextUpdateTick - lastTick >= roleTextRefreshInterval;

                    if (!shouldRefreshRoleText)
                    ...
                    lastRoleTextUpdateTick[__instance.PlayerId] = roleTextUpdateTick;
...

                    {
                        ...
                        // ブラックビジョナー/ブラックマッドメイトから見た他プレイヤーの名前を非表示にする(自分自身は除く)

                        || ((seer.Is(CustomRoles.BlackVisioner) || seer.Is(CustomRoles.BlackMadMate)) && target.PlayerId != seer.PlayerId && !TemporaryName))
