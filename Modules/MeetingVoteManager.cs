...
public class MeetingVoteManager
{
    ...
    public void EndMeeting(bool applyVoteMode = true, bool ClearAndExile = false)
    {
        ...
        // Nautilus等が参照する「直近の会議での投票先」を更新する。
        // 前回分は今回の会議の内容で完全に上書きする(会議に参加した全員分を記録し直す)。
        Main.LastMeetingVotedFor = new();
        foreach (var vote in AllVotes.Values)
        {
            Main.LastMeetingVotedFor[vote.Voter] = vote.VotedFor;
        }
            ...
            foreach (var player in PlayerCatch.AllPlayerControls)
{
    ...
    public static void ResetVoteManager(byte killtargetid)
    {
        ...
        _ = new LateTask(() =>
        {
            ...
            // MeetingHudやPlayerVoteAreaは、2秒後には既にDestroy済みになっている場合がある。
            // Destroy済みのUnityObjectはC#的にはnullではない(?.では素通りしてしまう)ため、
            // 明示的に == null で判定してから使う必要がある。
            // (これを怠ると、ここでNullReferenceExceptionが発生し、
            //  以降の切断処理やロビー復帰処理まで巻き込んで壊れることがある)
            if (meetingHud == null) return;
            if (meetingHud.playerStates == null) return;
            foreach (var x in meetingHud.playerStates)
            {
                if (x == null) continue;
                if (x.MaskArea == null) continue;
                if (x.MaskArea.gameObject == null) continue;
                x.MaskArea.gameObject.SetActive(true);
            }
        }, 2f, "ResetMask");
