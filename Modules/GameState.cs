...

{
...

    {
...

        {
            ...
            if (Main.DebugVersion) Logger.Info($"{player.GetNameWithRole().RemoveHtmlTags()}: UpdateTask", "TaskState.Update");

            GameData.Instance.RecomputeTaskCounts();

            //PlayerControl.CompleteTask Prefixから呼ばれるのでGameDataとは1ずれている

            if (Main.DebugVersion) Logger.Info($"TotalTaskCounts = {GameData.Instance.CompletedTasks + 1}/{GameData.Instance.TotalTasks}", "TaskState.Update");
            ...
            if (Main.DebugVersion) Logger.Info($"{player.GetNameWithRole().RemoveHtmlTags()}: TaskCounts = {CompletedTasksCount}/{AllTasksCount}", "TaskState.Update");
