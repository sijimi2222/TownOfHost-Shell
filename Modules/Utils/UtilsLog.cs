...

{
...

    {
...

        {
            ...
            // 詳細設定の全件ダンプは通常試合のログを数千行単位で増やすため、

            // デバッグ版でのみ出力する。ログ保存・フォルダ表示の機能は通常版でも維持する。

            if (Main.DebugVersion && Main.GameCount == 0 && !GameStates.IsNotJoined)
...

    {
...

        {
            ...
            sb.Append(string.Format(GetString("LastResult.GameCount"), Main.GameCount)).Append("</size>");
