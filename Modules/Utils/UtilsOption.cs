...

{
...

    {
        ...
        /// "/cmd n r"の第3引数からカテゴリ絞り込み文字を判定する。

        /// I/M/C/N/A/G/O(大文字小文字問わず)以外はnull(絞り込み無し)を返す。

        /// </summary>

        public static string ParseRoleCategory(string arg)
        ...
        public static void ShowActiveRoles(byte PlayerId = byte.MaxValue, bool IsMonochrome = false, string category = null)

        {
            ...
            var m = GetActiveRoleText(PlayerId, category);
        ...
        /// カテゴリ絞り込み用の文字("I","M","C","N","A","G","O")。nullなら絞り込み無し(従来通り全部表示)。

        /// I=インポスター M=マッドメイト C=クルー N=ニュートラル A=属性(アドオン。幽霊系除く) G=ゴースト系アドオン

        /// O=その他(コンビネーション役職・スロット役職など、上記6分類に当てはまらないもの)

        /// </summary>

        public static string GetActiveRoleText(byte pc, string category = null)

        {
...

                {
...

                    {
...

                        {
                            ...
                            var wantedType = category switch

                            {
                                ...
                                _ => (CustomRoleTypes?)null

                            };

                            if (wantedType == null) continue; // A/G/O絞り込み時は通常役職セクションを丸ごとスキップ

                            if (role.GetCustomRoleTypes() != wantedType) continue;
            ...
            if (addons != null && addons?.Length != 0 && (category == null || category == "A" || category == "G"))

            {

                var filteredAddons = addons.Where(a => a.IsEnable() && Event.CheckRole(a));

                if (category == "A") filteredAddons = filteredAddons.Where(a => !a.IsGhostRole());

                if (category == "G") filteredAddons = filteredAddons.Where(a => a.IsGhostRole());
...

        {
...

            {
                ...
                CustomRoles.LastCrewmate => AddonInfo(role, from: From.TownOfHost_hamo),
