...
namespace TownOfHost
{
    public static class AntiBlackout
    {
        ...
        public static void ResetSetRole(PlayerControl Player, int retryCount = 0)
        {
            ...
            {
                // clientが一時的にnullな場合(回線が瞬断している場合など)、
                // ここで諦めてしまうと、その人の本来の役職への復帰(暗転解除)が
                // 永久に行われなくなってしまう(マッド等が延々と暗転し続けるバグの原因)。
                // 一定回数までは少し待って再試行する。
                // ただし、本当に切断済みの場合は再試行しても意味が無いため打ち切る。
                bool actuallyDisconnected = Player?.Data?.Disconnected ?? true;
                const int maxRetry = 5; // 0.5秒間隔で最大2.5秒ほど再試行する
                if (!actuallyDisconnected && retryCount < maxRetry)
                {
                    Logger.Warn($"{Player?.Data?.PlayerName ?? "???"}のclientが一時的にnullのため、{retryCount + 1}回目の再試行を予約します", "ExiledSetRole");
                    _ = new LateTask(() => ResetSetRole(Player, retryCount + 1), 0.5f, "ResetSetRoleRetry", true);
                }
                else
                {
                    Logger.Error($"{Player?.Data?.PlayerName ?? "???"}のclientがnull(再試行上限 or 切断済みのため中止)", "ExiledSetRole");
                }
                return;
            }
