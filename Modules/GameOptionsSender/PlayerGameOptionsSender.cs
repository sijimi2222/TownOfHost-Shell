...

        {
...

            {
                ...
                if (player.Is(CustomRoles.GM))
                    ...
                    HaveWatching |= OptionGMCanSeeVotes.GetBool();

                else if ((GhostCanSeeOtherVotes.GetBool() || !GhostOptions.GetBool()) && !player.Is(CustomRoles.AsistingAngel) && (!player.IsGhostRole() || GhostRoleCanSeeOtherVotes.GetBool()))
