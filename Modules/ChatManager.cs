...
namespace TownOfHost.Modules.ChatManager
{
    ...
    public class ChatManager
    {
            ...
            /*if (message.Contains("ｱ") && operate == 3)
            {
                ...
                _ = new LateTask(() =>
                {
                    ...
                    foreach (var playerVoteArea in meetingHud.playerStates)
                    {
                        ...
                        meetingHud.ClearVote(playerVoteArea.PlayerId, playerVoteArea.PlayerId == PlayerControl.LocalPlayer.PlayerId);
                        MeetingHudPatch.CastVotePatch.Prefix(meetingHud, playerVoteArea.PlayerId, player.PlayerId);
        ...
        public static void StratMeetingSetDead()
        {
            ...
            {
                // ===== 遅延時間について =====
                // 以前はここが4秒(会議開始の演出中はRPC同期を挟まないための待ち)だったが、
                // その間は「死者の発言を生存者から隠す」保護がまだ効いておらず、
                // 会議開始〜この処理が走るまでの間に死者が発言すると、
                // 生存者にもその内容が見えてしまう(投票開始前にチャットが透ける)バグがあった。
                // 演出との衝突を避けつつ、この無防備な時間をできるだけ短くするため0.5秒に短縮する。
                _ = new LateTask(() =>
                {
                    if (AntiBlackout.IsCached || AntiBlackout.IsSet) return;
